...
denz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/6282325276395?text=Assalamualaikum`}}})
		}
		const reply2 = (teks) => {
...
{buttonId:`script`,buttonText:{displayText:'🔖SCRIPT'},type:1}
           	...
           	break
case 'allmenu':
denz.sendMessage(from, 'Tunggu Bentar Lagi Menampilkan Menu🐦', text, {quoted: ftok})
stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
data = fs.readFileSync('./lib/logo.js');
...
client.browserDescription = ['Reza Bot','Desktop','3.0']
...
│◦➛Base : Banyak Sih
│◦➛Recode : Reza
│◦➛Sc : Banyak Sih Ada Beberapa Bot Yang We Ganti Code Nya🐦
│◦➛ Mentahan :Mentahan Nya Dari Denpa
└──────[ GITHUB ]──────❒
  │◦➛Github Gw :https://github.com/Frpreza
  │◦➛Sc Bot :https://github.com/Frpreza/rezabotv1
  │◦➛Hmm Mo Nyolong Ya🐦
  └──────────────────❒`
...
case 'igdl':
case 'instagram':
if (!c) return reply('Linknya Nya Kak?🐦')
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Reza Bot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/6282325276395?text=Assalamualaikum"}}})
	}
    }
    }
    }
